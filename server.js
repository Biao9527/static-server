...
var server = http.createServer(function(request, response){
  ...
  if(path === '/sign_in' && method === 'POST'){
    ...
    let sessionId
    try {
      sessionId = cookie.split(';').filter(s=> s.indexOf('user_id=')>=0)[0].split('=')[1]
    } catch (error) {
    }
    if (sessionId && session[sessionId]) {
      ...
      let string
      if (user) {
        string = homeHtml.replace('{{loginStatus}}' , '已登录').replace('{{user_name}}',user.name)
      }else{
        string = homeHtml.replace('{{loginStatus}}' , '未登录').replace('{{user_name}}','')
      }
      response.write(string)
    } else {
      const homeHtml = fs.readFileSync('public/home.html').toString()
      const string = homeHtml.replace('{{loginStatus}}' , '未登录').replace('{{user_name}}','')
      response.write(string)
    }
    response.end()
